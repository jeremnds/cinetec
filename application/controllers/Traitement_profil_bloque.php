<?php 
class Traitement_profil_bloque extends CI_Controller{
    
    public function index(){
   $this->load->model('Traitement_profil_bloque_model');
        ...
        $this->session->userdata();
         ...
         $data['select_archive']=$this->Traitement_profil_bloque_model->select_archive($_SESSION['id']);
         $data['select_last']=$this->Traitement_profil_bloque_model->select_last($_SESSION['id']);
          $data['select_user']=$this->Traitement_profil_bloque_model->select_user($_SESSION['id']);
        ...
        $this->form_validation->set_rules('mail', 'Mail', 'trim|valid_email|callback_check_mail');
        ...
        /** PARTIE OBLIGATOIRE SI LES REGLES NE SONT PAS RESPECTEES (false) bref lol on se comprends sans ça le formulaire ne fonctionne pas **/
         ...
         $this->Traitement_profil_bloque_model->update($name,$firstname,$mail,$password, $login,$id); 
        ...
        $this->load->view('Traitement_profil_bloque_view');
  ...
  $reponse = $this->Traitement_profil_bloque_model->mailsess($mailsess);      
...
return true;
  ...
  $reponse2 = $this->Traitement_profil_bloque_model->loginsess($loginsess);      
